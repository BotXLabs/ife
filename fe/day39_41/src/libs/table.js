...
import { getQueryArray } from './hash'
...
export function getData() {
  ...
  let cbxCheckedValList = getQueryArray()
  let filterArray = []

  for (let val of cbxCheckedValList) {
    if (REGION_MAP[val]) {
      filterArray.push(REGION_MAP[val])
    } else if (PRODUCT_MAP[val]) {
      filterArray.push(PRODUCT_MAP[val])
    }
  }
